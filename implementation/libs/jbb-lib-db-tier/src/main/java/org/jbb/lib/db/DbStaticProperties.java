...
@Config.Sources({"file:${jbb.home}/jbb-lib-db-tier-STATIC.properties"})
public interface DbStaticProperties extends ModuleStaticProperties {
    ...
    @Config.Key(DB_FILENAME_KEY)
    String dbFilename();

    @Config.Key(DB_MIN_IDLE_KEY)
    int minimumIdle();

    @Config.Key(DB_MAX_POOL_KEY)
    int maxPool();

    @Config.Key(DB_CONN_TIMEOUT_MS_KEY)
    int connectionTimeoutMiliseconds();

    @Config.Key(DB_INIT_FAIL_FAST_KEY)
