...
public class AcpAdministratorPermissionRoleDetailsControllerIT extends BaseIT {
    ...
    public void shouldPutRoleDetails_intoModel_whenGet() throws Exception {
        ...
        // when
        ResultActions result = mockMvc.perform(get("/acp/permissions/role-administrators/details")
                .param("id", "12"));
    ...
    public void shouldPutPredefinedRole_intoModel_whenGetNewRole() throws Exception {
        ...
        // when
        ResultActions result = mockMvc.perform(get("/acp/permissions/role-administrators/new"));
    ...
    public void shouldFillModelWithPermissions_whenCreateOrEditRoleDetails() throws Exception {
        ...
        // when
        ResultActions result = mockMvc.perform(post("/acp/permissions/role-administrators/new/details")
                .param("roleId", "12"));
    ...
    @Test
    public void shouldDoesNotAcceptRoleWithoutName() throws Exception {
        // given
        prepareMockForRoleDefAndPermissionTable();

        // when
        ResultActions result = mockMvc.perform(post("/acp/permissions/role-administrators")
            .param("definition.name", "")
            .param("valueMap.CAN_ADD_FORUMS", "YES")
        );

        // then
        result.andExpect(status().isOk())
            .andExpect(view().name("acp/permissions/role-details"));
        Mockito.verifyNoMoreInteractions(permissionRoleServiceMock);
    }
    ...
    public void shouldInvokeAddRoleServiceMethod_whenCreateRole() throws Exception {
        ...
        result.andExpect(status().is3xxRedirection())
            .andExpect(view().name("redirect:role-administrators"));
    ...
    public void shouldRemoveRole_whenDelete_andRedirect() throws Exception {
        // when
        ResultActions result = mockMvc.perform(post("/acp/permissions/role-administrators/delete")
                .param("id", "12"));
    ...
    public void shouldMoveUp_andRedirect() throws Exception {
        // when
        ResultActions result = mockMvc.perform(post("/acp/permissions/role-administrators/moveup")
    ...
    public void shouldMoveDown_andRedirect() throws Exception {
        // when
        ResultActions result = mockMvc.perform(post("/acp/permissions/role-administrators/movedown")
                .param("id", "12").param("position", "4"));
    ...
    private void prepareMockForRoleDefAndPermissionTable() {
        ...
        given(permissionDefinition.getCode()).willReturn(AdministratorPermissions.CAN_ADD_FORUMS.getCode());
        given(permissionDefinition.getCategory()).willReturn(AllPermissionCategories.FORUMS);
