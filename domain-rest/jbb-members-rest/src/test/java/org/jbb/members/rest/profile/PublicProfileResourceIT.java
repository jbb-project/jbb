...
public class PublicProfileResourceIT extends BaseIT {
    ...
    @Test
    public void shouldGetPublicProfile_ifMemberExists() throws MemberNotFoundException {
        // given
        ...
        // when
        MockMvcResponse response = request.when().get("/api/v1/members/200/public-profile");

        // then
        response.then().statusCode(200);
        ProfilePublicDto resultBody = response.then().extract().body().as(ProfilePublicDto.class);
        assertThat(resultBody.getDisplayedName()).isEqualTo("Jacob");
    }

    @Test
    public void shouldHandleMemberNotFoundException() throws MemberNotFoundException {
        // given
        given(memberServiceMock.getMemberWithIdChecked(any()))
        ...
        assertThat(resultBody.getCode()).isEqualTo(ErrorInfo.MEMBER_NOT_FOUND.getCode());
    }
}