...
import static org.jbb.permissions.rest.PermissionsRestAuthorize.IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_SCOPE;
import static org.jbb.permissions.rest.PermissionsRestAuthorize.IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_WRITE_SCOPE;
...
@Api(tags = API_V1 + MEMBER_PERMISSIONS)
@RequestMapping(value = API_V1 + MEMBER_PERMISSIONS, produces = MediaType.APPLICATION_JSON_VALUE)
public class MemberPermissionResource {

    @GetMapping
    @ErrorInfoCodes({UNAUTHORIZED, FORBIDDEN})
    @ApiOperation("Gets member permission matrix for identity")
    @PreAuthorize(IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_SCOPE)
    ...
    @ErrorInfoCodes({UNAUTHORIZED, FORBIDDEN})
    @ApiOperation("Gets effective member permission matrix for identity")
    @PreAuthorize(IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_SCOPE)
    ...
    @ErrorInfoCodes({UNAUTHORIZED, FORBIDDEN})
    @ApiOperation("Gets available member permission definitions")
    @PreAuthorize(IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_SCOPE)
    ...
    @PutMapping
    @ErrorInfoCodes({UNAUTHORIZED, FORBIDDEN})
    @ApiOperation("Updates member permission matrix for identity")
    @PreAuthorize(IS_AN_ADMINISTRATOR_OR_OAUTH_PERMISSION_MEMBER_READ_WRITE_SCOPE)
